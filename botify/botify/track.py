...
class Catalog:
    ...
    def load(self, catalog_path, top_tracks_path, tracks_with_diverse_recs_path, catalog_new_way_path):
        ...
        self.app.logger.info(f"Loading tracks from {catalog_new_way_path}")
        with open(catalog_new_way_path) as catalog_file:
            for j, line in enumerate(catalog_file):
                data = json.loads(line)
                self.tracks_new_way.append(
                    Track(
                        data["track"],
                        data["artist"],
                        data["title"],
    ...
    def upload_tracks(self, redis_tracks, redis_tracks_with_diverse_recs, redis_tracks_new_way):
        ...
        for track in self.tracks_new_way:
            redis_tracks_new_way.set(track.track, self.to_bytes(track))
